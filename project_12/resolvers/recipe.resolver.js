...
const recipeResolver = {
    ...
    Mutation: {
        ...
        deleteRecipe: async (parent, {id}, contextValue) => {
        ...
        editRecipe: async (parent, {id, recipeInput: {name, description}}, {user}) => {
            const isExists = await RecipeHelper.isRecipeExists(id);
            if (!isExists) {
                throwCustomError(
                    `Recipe with id ${id} does not exists.`,
        ...
        incrementThumbsUp: async (parent, {id}, {user}) => {
            const isExists = await RecipeHelper.isRecipeExists(id);
            if (!isExists) {
                throwCustomError(
                    `Recipe with id ${id} does not exists.`,
        ...
        incrementThumbsDown: async (parent, {id}, {user}) => {
            const isExists = await RecipeHelper.isRecipeExists(id);
            if (!isExists) {
                throwCustomError(
                    `Recipe with id ${id} does not exists.`,
